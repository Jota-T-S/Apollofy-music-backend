...
export const getSearchResults = async (
  ...
  try {
    // find tracks
    const tracks = await TrackModel.find({name: {$regex: inputValue, '$options' : 'i'}}).lean().exec();
    // find playlists
    const playlists = await PlaylistModel.find({name: {$regex: inputValue, '$options' : 'i'}}).lean().exec();
